...
# oprHistory may monitor same operation twice, check that
...
# above SQL infos may vary
self.mydb = mysql.connector.connect (
    database = "Q18khiSkUF", # respective part for implementation
    host = "remotemysql.com",
    user = "Q18khiSkUF",
    password = "vYk8e6uPE0" )

nonenglishChar = ["ğ", "ü", "ş", "İ", "ı", "ö", "ç", "Ğ", "Ü", "Ş", "Ö", "Ç"]
# manipulate below statement whether it is password or name or surname
# remotemysql database accepts UTF-8 format so no need to that statement
elif any(char in nonenglishChar for char in self.passw): # that is general usage
    isuser = False
    print("Your password can NOT have non-English character(s)\n")
    time.sleep(1)

# | THIS IS FOR ONLINE VERSION OF APPLICATION |
"""
...
class BankCore:
    ...
    def createUser(self, name, sname, id, passw, iban):
        ...
        try:
            ...
            act = (1, None, None, None, None, None, None, None, None, None, None) # actions are left as 'NULL'
        ...
        try:
            ...
            values = (1, name, sname, id, passw, "0", "0", "0", "0", "0", "0", "0", iban, "0")
    ...
    def nameCorrection(self):
        ...
        if(len(self.name) > 13 or len(self.name) < 2):
            ...
            time.sleep(1)
        # elif any(char in nonenglishChar for char in self.name): # that is general usage
        #     isuser = False
        #     print("Your name can NOT have non-English character(s)\n")
        #     time.sleep(1)
    ...
    def snameCorrection(self):
        ...
        if(len(self.sname) > 15 or len(self.sname) < 2):
            ...
            time.sleep(1)
        # elif any(char in nonenglishChar for char in self.sname): # that is general usage
        #     isuser = False
        #     print("Your surname can NOT have non-English character(s)\n")
        #     time.sleep(1)
    ...
    def idCorrection(self): 
        ...
        isuser = True
        if not all(char in nums for char in self.id):
            isuser = False
            print("Identity number only consist of integers\n")
            time.sleep(1)
            self.idCorrection()
        elif not(int(len(self.id)) == 11):
            isuser = False
        ...
        if(x[-1:] == self.id[10] and self.id[-1] == x[-1] and q1[-1] == self.id[9] and q2[-1] == self.id[9]):
            ...
            isuser = False
            time.sleep(1)
        else: # to check if anything wrong
        ...
        for data in self.database:
            if(toUser == str(data[12])):
                ...
                if(str(checkBalance) == "(1,)" or tlpass == True): # couldn't convert checkBalance boolean, found a way like this
                    ...
                    while(ismoney == False):
                        try:
                            print(f"Transfer fee: 0.00 {moneytype}")
            ...
            for o in operations:
                while i <= 10:
                    ...
                    if o[i] != "on" and o[i] != None:
                        ...
                        if(operations[5] == "N" and i == 9): # if act1 is "NULL" this will be printed once 
